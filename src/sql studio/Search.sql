...
ScoredWords AS (
	SELECT
		...
		(
			...
			-- Word length penalty
			- (ABS(m.QueryWordLength - dw.WordLength) / CAST(GREATEST(m.QueryWordLength, dw.WordLength) AS FLOAT)) * 0.3
...
ScoredEntities (EntityId, Score) AS (
	SELECT wi.EntityId, SUM(sw.Score) AS Score
	FROM ScoredWords sw
	JOIN WordInstance wi ON wi.WordId = sw.WordId
	GROUP BY wi.EntityId
)
SELECT TOP 20 *
FROM ScoredEntities se
JOIN LabelEntity le ON le.Id = se.EntityId
ORDER BY Score DESC